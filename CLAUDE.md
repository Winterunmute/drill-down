...
2. `js/engine.js` — `DrillDown.Engine`. Pure game logic, no DOM. Grid ops, stat computation, run simulation, adjacency synergies, shop generation, recycling/fragments, and versioned localStorage save/load + migration.
...
### Screens
The UI is a set of `.screen` divs in `index.html` (`screen-title`, `screen-workshop`, `screen-drill`) plus two overlays (`shop-overlay`, `help-overlay`). There is no separate results screen — results are rendered inline on the drill screen after the run completes. `UI.showScreen(id)` toggles the `active` class. Each screen is (re)rendered imperatively by a `render*` function that rewrites `innerHTML` and re-attaches event handlers — there is no virtual DOM or framework. **Boot is quick-start:** `Game.init()` renders the title but immediately `continueRun()`s into the workshop if a save exists; the workshop's "⌂ Title Menu" button (and the title's New Run, which pops `UI.confirmModal` when a save exists) are the way back. The workshop inventory is grouped into labeled type sections (drill/cooling/defense/utility/core); within a section, a base part and its Mk tiers stay adjacent, families ordered by base rarity. Duplicate stacks show a `×N` badge plus the Mk-combine button. `UI.confirmModal(opts)` is a reusable themed replacement for native `confirm()`.

### Game state
A single plain object (see `defaultState()` in `js/main.js`): `gold`, `grid`, `inventory` (array of part IDs, duplicates allowed), `fragments` (partId → count toward crafting), `recycleProgress` (0–100 salvage meter), `returnPolicy` (`{cargoFull, hpPct}` auto-surface settings), `shop` (array of part IDs for sale), and run stats (`runNumber`, `bestDepth`, etc.). It's persisted to `localStorage` under the key `drill_down_save` on nearly every mutation via `Eng.save(state)`. Saves are versioned (`Engine.SAVE_VERSION`): `Eng.load()` runs `Eng.migrate()`, which fills any missing/invalid fields with defaults and prunes references to parts that no longer exist — so adding a state field or removing a part never breaks an existing save. Keep `migrate`'s field defaults in sync with `defaultState()`. `state` lives only in `Game`; access it elsewhere through `DrillDown.Game.state`.
...
### Stats & adjacency
`Engine.computeStats(grid)` sums each placed part's `stats` into a robot stat block (`drillPower`, `heatGen`, `cooling`, `hp`, `armor`, `cargo`, `speed`, `detect`), then applies adjacency synergies (cooling-next-to-drill, drill-next-to-drill, defense-next-to-defense, defense-next-to-drill, utility-next-to-drill/cooling, utility-next-to-utility) and core amplification (`def.amp` fraction of adjacent parts' primary stat). It then applies **soft power-limiting** (`SOFT_CAPS` + `softCap`): a logarithmic diminishing-returns curve above a per-stat threshold, so over-stacking yields flattening (not trivial) depth scaling. The curve is smooth at the threshold and always ≤ raw (never boosts) yet unbounded. Only `drillPower`, `hp`, and `armor` are capped — `cooling` is deliberately left linear so it can always offset the uncapped `heatGen` of big builds (capping it would invert progression). Floors are applied last. This block is recomputed fresh whenever needed; it is not stored in state.
...
### Economy & progression
Ore converts to gold on run completion (×2, and only if the drone surfaces). Shop restocks via `Engine.generateShop(runNumber, bestDepth)`, whose rarity mix **scales with best depth** (`Engine.shopPlan`): from 3 common / 2 uncommon / 1 rare at the surface toward rare-heavy stock deeper, plus a single premium `unique` slot at best depth ≥ 300 (priced at the flat `SHOP_UNIQUE_PRICE` via `Engine.shopCost`, since unique `cost` is 0). Fragments from "ancient caches" remain the *cheaper* route to uniques: they accumulate until enough to craft a full part (2 for rare, 3 for unique). Uniques are deliberately rare: `Engine.randomCraftable(uniqueChance)` defaults to `UNIQUE_FRAGMENT_CHANCE` (12%) for recycle awards, and `getEvent` ramps the cache unique chance with depth (≈0 near surface → capped 18% deep) — most fragments are rares. `Engine.addFragment(state, partId)` centralizes fragment accrual + auto-craft. Recycling a part (`Engine.recyclePart`) grants a little gold and fills `recycleProgress`; at 100% it awards a fragment of a random rare/unique part via `addFragment`.

**Mk II/III/IV upgrades:** every non-unique part has an auto-generated tier chain `<id>_mk2`/`_mk3`/`_mk4` (built in `js/parts.js`'s `generateUpgrades` IIFE — good stats ×1.6/tier, heat ×1.3/tier so each tier is more heat-efficient, trade-off penalties fixed, core `amp` +0.15/tier; rarity climbs and caps at rare). Each tier carries `upgradeOf` (back) and `upgradeTo` (forward), forming base → Mk II → Mk III → Mk IV; the top tier has no `upgradeTo`. Combining two copies of a tier + gold (`Engine.upgradeCost`/`canUpgrade`/`upgradePart`) yields the next tier. Mk parts are **combine-only**: anything with an `upgradeOf` field is filtered out of the shop, ancient-cache fragments, and recycle award pools. The workshop shows an `⬆ Mk II/III/IV` button (label from the target's `tier` field) on any stacked inventory card whose part has an `upgradeTo`.

## Conventions
- Pure vanilla JS, no modules/imports — everything via the `DrillDown` global. New code should attach to it the same way.
- Adding a part = add one entry to `DrillDown.PARTS` in `js/parts.js`. Fields: `id`, `name`, `type` (`drill`/`cooling`/`defense`/`utility`/`core`), `shape`, `stats`, `rarity` (`common`/`uncommon`/`rare`/`unique`), `cost`, `color`, `emoji`. The shop, tooltips, and shape rendering all read from this automatically. Any part may include an optional `desc` string shown as flavor / trade-off text in the tooltip. `core`-type parts also need an `amp` field (e.g. `0.25`) and a `desc`; their `stats` can be empty. Stats may be negative (part trade-offs); `computeStats` clamps the final block to sane floors.
